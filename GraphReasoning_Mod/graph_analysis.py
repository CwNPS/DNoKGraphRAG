...
This module contains functions for graph analysis in support of GraphRAG.

Modified by Jonathan Kasprisin 

References: https://github.com/lamm-mit/GraphReasoning
"""
...
def heuristic_path_with_embeddings(G, embedding_object, source, target, node_embeddings, top_k=3, second_hop=False,
                                   data_dir='./', save_files=True, verbatim=False,):
    G = deepcopy(G)

    if verbatim:
        print ("Original: ", source, "-->", target)
    source=find_best_fitting_node_list(source, node_embeddings, embedding_object, 5)[0][0].strip()
    target=find_best_fitting_node_list(target, node_embeddings, embedding_object, 5)[0][0].strip()
...
def find_shortest_path (G,source='graphene', target='complexity', verbatim=True, data_dir='./'):
    """
    Note: Commented out visualization and save files 
    """
...
def find_N_paths (G, source, target, N=5, data_dir='./temp/function_outputs'):
...
def find_path( G, node_embeddings,  embedding_object, keyword_1 = "", keyword_2 = "", 
            verbatim=True, second_hop=False,data_dir='./', similarity_fit_ID_node_1=0,
            similarity_fit_ID_node_2=0,save_files=True):
...
def find_path_and_reason (G, node_embeddings,  embding_object, generate, 
                          keyword_1,
                          keyword_2, include_keywords_as_nodes=True,
                          inst_prepend='',
                          ...
                          similarity_fit_ID_node_1=0, similarity_fit_ID_node_2=0, #which path to include 0=only best, 1 onlysecond best, etc.
    ...
    (best_node_1, best_similarity_1, best_node_2, best_similarity_2), path, path_graph, shortest_path_length, fname, graph_GraphML=find_path( G,node_embeddings, embding_object, keyword_1 = keyword_1,  keyword_2 = keyword_2,verbatim=verbatim,
    ...
    if visualize_paths_as_graph:
        path_list_for_vis, _=print_path_with_edges_as_list(G, path, keywords_separator=keywords_separator)                                                                                                                                        
    ...
    if contains_phrase( graph_analysis_type, 'path and relations') :
        if N_limit != None:
            path=path[:N_limit]
        _, path_list_string= print_path_with_edges_as_list(G, path, keywords_separator=keywords_separator)
...
def find_path_with_relations_and_reason_combined(G, node_embeddings, embedding_object, generate,
    ...
    if include_all_possible:
        ...
        for start_id in range(num_paths):
            for end_id in range(num_paths):
                ...
                paths_details.extend(process_path_combination(G, node_embeddings, embedding_object, keyword_1, keyword_2, verbatim, N_limit, keywords_separator, start_id, end_id, include_keywords_as_nodes,data_dir,save_files,visualize_paths_as_graph,display_graph=display_graph,words_per_line=words_per_line))
    else:
        # Process paths based on num_paths without considering all combinations
        for path_id in range(num_paths):

             
            paths_details.extend(process_path_combination(G, node_embeddings, embedding_object, keyword_1, keyword_2, verbatim, N_limit, keywords_separator, path_id, path_id, include_keywords_as_nodes,data_dir,save_files,visualize_paths_as_graph,display_graph=display_graph,words_per_line=words_per_line))
...
def process_path_combination(G, node_embeddings, embedding_object, keyword_1, keyword_2, verbatim, N_limit, keywords_separator, start_id, end_id, include_keywords_as_nodes,data_dir,save_files,
    ...
    paths_details = []
    (best_node_1, best_similarity_1, best_node_2, best_similarity_2), path, _, _, _, _ = find_path(
        G, node_embeddings, embedding_object, keyword_1=keyword_1,
...
def heuristic_path_with_embeddings_with_randomization_waypoints(G, embedding_object, source, target, 
    ...
    embedding_object : Callable
        Object used to generate embeddings for text data.
    ...
    if verbatim:
        print("Original: ", source, "-->", target)
    source_list = find_best_fitting_node_list(source, node_embeddings, embedding_object, 5)
    target_list = find_best_fitting_node_list(target, node_embeddings, embedding_object, 5)
